...
impl Bingo {
    ...
    pub fn bingo_on_board(&self, board: Vec<Vec<i32>>) -> bool {
    ...
    pub fn detect_bingo(&self, boards: Vec<Vec<Vec<i32>>>) -> (bool, Vec<Vec<i32>>) {
        ...
        for board in 0..boards.len() {
            if self.bingo_on_board(boards[board].clone()) {
                result = true;
    ...
    pub fn part_two(&mut self, contents: &str) -> i32 {
        ...
        for item in called_numbers {
           ...
           if boards.len() > 1 {
               let mut boards_without_bingo: Vec<Vec<Vec<i32>>> = vec![];
               for index_of_boards in 0..boards.len() {
                    if false == self.bingo_on_board(boards[index_of_boards].clone()) {
                        boards_without_bingo.push(boards[index_of_boards].clone());
                    }
               }
               boards = boards_without_bingo;            
           }
           ...
           if boards.len() == 1 {
                ...
                if bingo_happened {
                    self.lastcalled = item;
                    self.unmarked = self.total_solution(solution_board.clone());
                    return self.lastcalled * self.unmarked;
                }
            }
        }
...
22 11 13  6  5
 2  0 12  3  7";
        let mut bingo = Bingo::new();
        let answer = bingo.part_two(&contents);
        assert_eq!(148, bingo.unmarked);
        assert_eq!(13, bingo.lastcalled); 
